...
def main(hydra_cfg: DictConfig):
    ...
    run_name = getattr(hydra_cfg, "run_name", "default")
    save_suffix = getattr(hydra_cfg, "save_suffix", None)
    ...
    for idx_model, model_keyname in enumerate(all_eval_models, start=1):
        ...
        for idx_dataset, dataset_name in enumerate(all_eval_datasets, start=1):
            ...
            model_logger.info(f"Save point clouds/images: {save_outputs}")
            ...
            if num_samples == 0:
                raise RuntimeError(f"No samples were evaluated for {dataset_name} with {model_keyname}")
            metric_dict = {
                metric: value / num_samples
                for metric, value in all_data_dict.items()
                if metric not in {"model", "run_name", "save_suffix", "dataset"}
            }
            model_logger.info(
                "Overall %s | model=%s | run_name=%s | "
                "Acc-mean=%.6f Acc-med=%.6f | "
                "Comp-mean=%.6f Comp-med=%.6f | "
                "NC-mean=%.6f NC-med=%.6f",
                dataset_name,
                model_keyname,
                run_name,
                metric_dict["Acc-mean"],
                metric_dict["Acc-med"],
                metric_dict["Comp-mean"],
                metric_dict["Comp-med"],
                metric_dict["NC-mean"],
                metric_dict["NC-med"],
            )
