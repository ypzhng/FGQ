...
from models.vggt.utils.rotation import mat_to_quat
...
def calculate_rra_rta(r_error, t_error, threshold=30):
    """
    Calculate Relative Rotation Accuracy (RRA) and Relative Translation Accuracy (RTA).

    :param r_error: numpy array of rotation errors in degrees.
    :param t_error: numpy array of translation errors in degrees.
    :param threshold: threshold in degrees (default: 30).
    :return: (rra, rta) as percentages.
    """
    rra = np.mean(r_error < threshold) * 100.0
    rta = np.mean(t_error < threshold) * 100.0
    return rra, rta
